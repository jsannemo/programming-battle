...
from battle.models import Session, Contest, Team, Problem, Solution, TestCase, SolutionEvent, Solution, Judgement
...
class JudgeDaemon:
    ...
    def start(self):
        sess = Session()
        while True:
            solutions = sess.query(Solution).from_statement("SELECT * FROM solution WHERE status = :queued ORDER BY solution_time ASC").params(queued=Status.queued.name).all()
            testcases = sess.query(TestCase).from_statement("SELECT * FROM testcase WHERE status = :queued ORDER BY testcase_time ASC").params(queued=Status.queued.name).all()
    ...
    def judge_testcase(self, sess, testcase):
        ...
        solutions = sess.query(Solution).from_statement("SELECT * FROM solution WHERE problem_id = :problem AND solution_time < :submission AND status = :active ORDER BY solution_time ASC") \
            .params(problem=testcase.problem.problem_id, submission=testcase.testcase_time, active = Status.active.name).all()
        ...
        for solution in solutions:
            ...
            if not judgement.solved:
                self.transition(sess, solution, testcase, Status.defeated.name, testcase.testcase_time)
    ...
    def judge_solution(self, sess, solution):
        ...
        test_cases = sess.query(TestCase).from_statement("SELECT * FROM testcase WHERE problem_id = :problem AND testcase_time <= :submission AND status = :active ORDER BY testcase_time ASC") \
            .params(problem=problem.problem_id, submission=solution.solution_time, active=Status.active.name).all()

        self.transition(sess, solution, None, Status.testing.name, solution.solution_time)
        ...
        if not program.compile():
            self.transition(sess, solution, None, Status.rejected.name, solution.solution_time)
        ...
        for testcase in test_cases:
            judgement = self.judge(sess, output_validator, program, solution, testcase)
            if not judgement.solved:
                self.transition(sess, solution, testcase, Status.failed.name, solution.solution_time)
                return

        previous = sess.query(Solution).filter_by(team=solution.team, problem=solution.problem, status=Status.active.name).all()
        for prev in previous:
            self.transition(sess, prev, None, Status.inactive.name, solution.solution_time)
        self.transition(sess, solution, None, Status.active.name, solution.solution_time)
